...
from tensorflow.keras.layers import Dense, BatchNormalization, ReLU, Input, LSTM, Concatenate, Masking, Reshape, Lambda, \
    Bidirectional, GRU, LayerNormalization, Bidirectional, Conv2D, Conv1D, MaxPooling2D, Flatten, LayerNormalization, \
    Layer, Embedding, MultiHeadAttention, Dropout
...
class spatial_softmax_cnn(tf.keras.Model):
    ...
    def __init__(self, img_height=128, img_width=128, img_channels=3, embedding_size=64, return_spatial_softmax=False):
        ...
        self.rescaling = Rescaling(1. / 255,
                                   input_shape=(img_height, img_width, img_channels))  # put it here for portability
        self.conv1 = Conv2D(32, 8, strides=(4, 4), padding='same', activation='relu', name='c1')
        self.conv2 = Conv2D(64, 4, strides=(2, 2), padding='same', activation='relu', name='c2')
...
class intensities_spatial_softmax_cnn(tf.keras.Model):
    ...
    def call(self, inputs):
        ...
        batch_indices = tf.reshape(tf.repeat(tf.range(0, N, 1)[tf.newaxis, :], C), [N, C])[:, :,
                        tf.newaxis]  # 0,0,0, 1,1,1, etc as batch indices
        keypoint_indices = tf.tile(tf.range(0, C, 1)[tf.newaxis, :], [N, 1])[:, :,
                           tf.newaxis]  # numbers 1,2,3... 1,2,3... keypoints, batches appropriately
        assert W == H  # this next step is currently only coded for squares
        keypoint_img_indices = tf.reverse(tf.cast(spatial_soft_argmax * W, tf.int32),
                                          [-1])  # gather nd has opposite axes to images, x is y, y is x
...
class deep_impala_cnn(impala_cnn):
    def __init__(self, img_height=128, img_width=128, img_channels=3, embedding_size=64, return_spatial_softmax=False):
        super(deep_impala_cnn, self).__init__(img_height, img_width, img_channels, embedding_size,
                                              return_spatial_softmax, l1=64, l2=128, l3=128)


CNN_DICT = {'spatial_softmax': spatial_softmax_cnn, 'intensities_spatial_softmax': intensities_spatial_softmax_cnn,
            'impala': impala_cnn, 'deep_impala': deep_impala_cnn}
