...
parser.add_argument('-tfr', '--from_tfrecords', default=False, action='store_true',
...
parser.add_argument('-la', '--actor_layer_size', default=2048, type=int,
                    help='Layer size of actor, increases size of neural net')
parser.add_argument('-le', '--encoder_layer_size', default=512, type=int,
                    help='Layer size of encoder, increases size of neural net')
parser.add_argument('-lp', '--planner_layer_size', default=2048, type=int,
                    help='Layer size of planner, increases size of neural net')
parser.add_argument('-lg', '--goal_mapper_layer_size', default=512, type=int,
                    help='Layer size of goal mapping networks from im and sent to goal space, increases size of neural net')
parser.add_argument('-embd', '--img_embedding_size', default=64, type=int, help='Embedding size of features,goal space')
parser.add_argument('-s_embd', '--sentence_embedding_size', default=512, type=int,
                    help='Embedding size of MUSE sentence embeddings')
parser.add_argument('-g_embd', '--gripper_img_embedding_size', default=32, type=int,
...
parser.add_argument('-g', '--gcbc', default=False, action='store_true',
                    help='Enables GCBC, a simpler model with no encoder/planner')
parser.add_argument('-n', '--num_distribs', default=None, type=int,
                    help='Number of distributions to use in logistic mixture model')
parser.add_argument('-q', '--qbits', default=None, type=int,
...
parser.add_argument('--codebook_size', type=int,
                    default=32)  # power of 2 by which the seq length is reduced to vq tiles
...
parser.add_argument('-enc_all', '--encode_all', default=False, action='store_true',
                    help='encode_actions_and_proprio not just imgs')
...
if args.data_source == 'GCS':
    ...
    logging.warning(
        f'You are accessing GCS data from {region}, make sure this is the same as your bucket {args.bucket_name}')
...
# # Dataset
dataset_coordinator = lfp.data.distributed_data_coordinator(args, TRAIN_DATA_PATHS, TEST_DATA_PATHS, strategy,
                                                            BULK_DATA_PATHS, VIDEO_DATA_PATHS, args.standard_split,
                                                            args.bulk_split, args.lang_split, args.video_split,
                                                            NUM_DEVICES)  # non-teleop, video only data

# # Model
from lfp.train import LFPTrainer

if args.device == 'CPU' or args.device == 'GPU':
    actor, encoder, planner, cnn, gripper_cnn, img_embed_to_goal_space, lang_embed_to_goal_space, trainer = lfp.train.train_setup(
        args, dataset_coordinator.dl, dataset_coordinator.GLOBAL_BATCH_SIZE, strategy=None)
else:
    with strategy.scope():
        actor, encoder, planner, cnn, gripper_cnn, img_embed_to_goal_space, lang_embed_to_goal_space, trainer = lfp.train.train_setup(
            args, dataset_coordinator.dl, dataset_coordinator.GLOBAL_BATCH_SIZE, strategy=strategy)
...
if args.resume:
    ...
    # Comet
    experiment = Experiment(api_key="C4vcCM57bnSYEsdncguxDW8pO", project_name="learning-from-play",
                            workspace="sholtodouglas")
