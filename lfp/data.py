...
def extract_tfrecords(paths, include_imgs=False, include_imgs2=False, include_gripper_imgs=False, sim='Unity',
...
class PlayDataloader():
    ...
    def __init__(self,
        ...
        if self.sim == 'Unity' and not self.include_imgs:
            print(
                'Confirm that data dimensions are correct - states will be cut down to exclude additional objects introduced for the image dataset - states has only been test on blue block.')
    ...
    def extract(self, paths, from_tfrecords=False):
        ...
        if from_tfrecords:
            ...
            random.shuffle(
                record_paths)  # to ensure good mixing of different time periods of data (important in the bulk dataset)
            dataset = extract_tfrecords(record_paths, self.include_imgs, self.include_imgs2, self.include_gripper_imgs,
                                        self.sim, ordered=True, num_workers=self.num_workers)
        else:
...
def serialise_traj(data):
    features = {k: Feature(bytes_list=BytesList(value=[tf.io.serialize_tensor(v).numpy(), ])) for k, v in data.items()
...
def load_traj_tf_records(filenames, read_func, include_imgs=True, include_imgs2=False, include_gripper_imgs=False,
                         ordered=False):
    ...
    dataset = dataset.map(read_func(include_imgs, include_imgs2, include_gripper_imgs),
...
class labelled_dl():
    ...
    def extract(self, filenames):
        ...
        return load_traj_tf_records(labelled_paths, self.read_func, self.include_images, self.include_images2,
                                    self.include_gripper_images)
...
class distributed_data_coordinator:
    ...
    def __init__(self,
                 ...
                 strategy,
                 BULK_DATA_PATHS=[],
                 # data we might want to emphaise in higher proportion - e.g if training for a standard viewpoint or environment
                 VIDEO_DATA_PATHS=[],
                 standard_split=64,
        ...
        self.bulk_split, self.standard_split, self.lang_split, self.video_split = int(bulk_split * NUM_DEVICES), int(
            standard_split * NUM_DEVICES), int(lang_split * NUM_DEVICES), int(video_split * NUM_DEVICES)
        print(
            f"Our dataset split is {self.standard_split} specific, {self.lang_split} lang, {self.video_split} video, {self.bulk_split} bulk")
        ...
        ######################################### Train
        self.dl = PlayDataloader(normalize=args.normalize, include_imgs=args.images, include_imgs2=args.images2,
                                 include_gripper_imgs=args.gripper_images, sim=args.sim,
                                 window_size=args.window_size_max, min_window_size=args.window_size_min)
        self.dl_lang = labelled_dl(sim=args.sim, include_images=args.images, include_images2=args.images2,
                                   include_gripper_images=args.gripper_images)  # this is probably fine as it is preshuffled during creation
        self.standard_dataset = iter(strategy.experimental_distribute_dataset(
            self.dl.load(self.dl.extract(TRAIN_DATA_PATHS, from_tfrecords=args.from_tfrecords),
                         batch_size=self.standard_split)))
        self.bulk_dataset = iter(strategy.experimental_distribute_dataset(
            self.dl.load(self.dl.extract(BULK_DATA_PATHS, from_tfrecords=args.from_tfrecords),
                         batch_size=self.bulk_split))) if self.bulk_split > 0 else None

        ######################################### Test
        valid_dataset = self.dl.load(self.dl.extract(TEST_DATA_PATHS, from_tfrecords=args.from_tfrecords),
                                     batch_size=self.bulk_split + self.standard_split)
        self.valid_dataset = iter(strategy.experimental_distribute_dataset(valid_dataset))

        ######################################### Plotting
        self.plotting_background_dataset = iter(
            self.dl.load(self.dl.extract(TEST_DATA_PATHS, from_tfrecords=args.from_tfrecords),
                         batch_size=32))  # for the background in the cluster fig
        ...
        if args.use_language:
            self.lang_dataset = iter(strategy.experimental_distribute_dataset(
                self.dl_lang.load(self.dl_lang.extract(TRAIN_DATA_PATHS + BULK_DATA_PATHS),
                                  batch_size=self.lang_split)))
            self.lang_valid_dataset = iter(strategy.experimental_distribute_dataset(
                self.dl_lang.load(self.dl_lang.extract(TEST_DATA_PATHS), batch_size=self.lang_split)))
        else:
    ...
    def next(self):
        ...
        lang = next(self.lang_dataset) if self.args.use_language else tf.constant(
            0.0)  # uing 0 constants as distribute strat hates none
        video = next(self.video_dataset) if self.args.use_contrastive else tf.constant(
            0.0)  # uing 0 constants as distribute strat hates none
        bulk = next(self.bulk_dataset) if self.bulk_split > 0 else tf.constant(
            0.0)  # combine batch and standard on device
    ...
    def next_valid(self):
        ...
        lang = next(self.lang_valid_dataset) if self.args.use_language else tf.constant(
            0.0)  # uing 0 constants as distribute strat hates none
        video = next(self.video_valid_dataset) if self.args.use_contrastive else tf.constant(
            0.0)  # uing 0 constants as distribute strat hates none
