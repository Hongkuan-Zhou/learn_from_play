    ...
    for k, v in test_labels.flatten()[0].items():
        ...
        if v not in hold_out:
            ...
            if args.images:
                ...
                imgs.append(traj_imgs), goal_imgs.append(traj_goal_img), proprioceptive_features.append(
                    traj_proprioceptive_features)
            acts.append(traj_acts), obs.append(traj_obs), goals.append(traj_goals), labels.append(v), colors.append(
                c), paths.append(TEST_DATA_PATH / "/".join(k.split('/')[4:]))

    return np.array(obs), np.array(acts), np.array(goals), labels, colors, paths, np.array(imgs), np.array(
        goal_imgs), np.array(proprioceptive_features)
...
def project_enc_and_plan(z_enc, z_plan, connecting_lines=False):
    ...
    z_plan = z_embed[l:]
    if connecting_lines:
        for i in range(len(z_enc)): plt.plot([z_enc[i, 0], z_plan[i, 0]], [z_enc[i, 1], z_plan[i, 1]], c='grey',
                                             linewidth=0.5)
